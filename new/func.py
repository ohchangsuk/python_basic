...
def main():
    step1()
    # 26번의 gameTitle과 step2() 안의 gameTitle은 서로 다른 변수다.
    # 그냥 편의상 이름만 동일하게 사용했다(이후 코드 안고치기 위하여 )
    gameTitle = step2()
    step3()
    step4()
...
def step2():
    ...
    # gameTitle은 정차적 코드에서는 그냥 사용해도 되나,
    # 함수 지향적으로 전개해서 함수 내부로 가면 지역변수가 된다.
    # 함수 밖에서 사용이 불가 하므로, 값을 리턴하거나, 아예
    # 전역변수로 빼야 한다.
    return gameTitle
def step3():
    while True:
...
#
    ...
    # # 1) python파일명.py로 구동하면 >> __name__>>'__main__'세팅됨
    # # 2) 누군가가 파일명.py를 가져와서 사용하면 => __name__ 은 파일명이됨
